...
for i in range(4):
    ...
    time_label.setStyleSheet("background-color: white; color: black; padding: 10px;")
...
def update_traffic_ui(lane, color, time):
    traffic_light_labels[lane].setText(color)
    traffic_light_labels[lane].setStyleSheet(f"background-color: {color.lower()}; color: white; padding: 10px;")
    time_remaining_labels[lane].setText(f"Time: {time}s")

traffic_manager.update_ui.connect(update_traffic_ui)
...
while all(cap.isOpened() for cap in caps):
    ...
    for i in range(4):
        vehicle_count_labels[i].setText(f"Feed {i + 1}: {vehicle_counts[i]} vehicles")
